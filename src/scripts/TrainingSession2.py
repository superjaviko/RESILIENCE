...
def request_training_sessions(operator_id):
    """Send a request to the external script and return only training sessions."""
    try:
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as client:
            client.settimeout(5)  # Evita que se quede colgado más de 5 segundos
            ...
            return response  # Retorna solo el training_sessions        
     
def update_training_sessions_info(cont):
